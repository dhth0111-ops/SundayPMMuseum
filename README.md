# SundayPMMuseum V2.0.4 Photo Cloud Sync
...
사진은 기기 IndexedDB와 전체 JSON 백업에 보관되며, Firestore에도 분할 저장되어 같은 계정의 다른 기기와 동기화됩니다.
...
- 사진을 Firestore의 사진 전용 하위 컬렉션에 안전한 크기로 분할 저장
- 자동 저장 시 사진 변경 여부를 확인해 필요한 경우에만 사진 재동기화
- 수동 클라우드 저장/불러오기에 사진 포함
- 기존 로컬 사진 저장 및 전체 JSON 백업 기능 유지
